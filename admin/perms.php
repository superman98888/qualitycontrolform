...
<?php if (isset($_POST['action']))
{
	...
	{
		case 'savePerm':
			$query = sprintf("REPLACE INTO `permissions` SET `ID` = %u, `permName` = '%s', `permKey` = '%s'",$_POST['permID'],$_POST['permName'],$_POST['permKey']);
			mysqli_query($conn, $query);
		break;
		case 'delPerm':
			$query = sprintf("DELETE FROM `permissions` WHERE `ID` = %u LIMIT 1",$_POST['permID']);
			mysqli_query($conn, $query);
		break;
	}
